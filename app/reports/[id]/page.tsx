  ...
  return (
    <AppShell authed>
      <article data-testid="report-page">
        <p className="text-xs font-medium tracking-[0.22em] text-outline">正式命名报告</p>
        <h1 className="mt-2 text-3xl font-semibold tracking-[0.12em] text-primary sm:text-4xl">
          命名结果
        </h1>
        ...
        <section data-testid="report-overview">
          ...
          <div className="mt-4">
            <p
              className="text-5xl font-semibold tracking-[0.28em] text-primary sm:text-6xl"
              data-testid="primary-name"
              style={{ textIndent: "0.28em" }}
            >
              {report.overview.primaryName}
            </p>
            <p className="mt-3 border-l-2 border-secondary pl-3 text-sm font-medium tracking-[0.2em] text-secondary">
              首推
            </p>
          </div>
          <p className="mt-4 max-w-[38ch] text-lg leading-relaxed text-on-surface-variant">
            ...
            {report.overview.names.map((n) => {
              ...
              return (
                <span
                  ...
                  className={
                    isPrimary
                      ? "inline-flex min-h-10 items-center border border-secondary bg-secondary/[0.07] px-3.5 py-1.5 text-base tracking-wider text-primary shadow-[inset_3px_0_0_0_#775a19]"
                      : "inline-flex min-h-10 items-center border border-outline-variant bg-surface px-3.5 py-1.5 text-base tracking-wider text-on-surface-variant"
                  }
                >
                  {n}
                  {isPrimary ? " · 首推" : ""}
          ...
          {report.preparationNote ? (
            <p className="mt-4 text-sm text-secondary">{report.preparationNote}</p>
          ) : null}
          ...
          <div className="mt-6 space-y-8">
            {report.names.map((n) => (
              <div
                ...
                className={
                  n.isPrimary
                    ? "border-l-[3px] border-secondary bg-gradient-to-r from-secondary/[0.06] to-transparent pl-4"
                    : "border-l-2 border-outline-variant/60 pl-4"
                }
              >
                <h3 className="text-2xl font-semibold tracking-[0.2em] text-primary">
                  {n.fullName}
                  {n.isPrimary ? (
                    <span className="ml-2 text-sm font-medium tracking-normal text-secondary">
                      首推
                    </span>
                  ) : null}
                  {n.l2Hot ? (
        ...
        <section data-testid="report-not-recommended">
          ...
          <ul className="mt-4 space-y-3">
            {report.notRecommended.map((e, i) => (
              ...
              >
                <span className="font-semibold tracking-wide text-primary">{e.givenName}</span>
        ...
        {report.bazi ? (
          <>
            ...
            <section data-testid="report-bazi">
              ...
              <p className="mt-4 text-lg tracking-widest text-primary">{report.bazi.pillarsText}</p>
